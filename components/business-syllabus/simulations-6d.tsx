...
import { Activity, Brain, Server, Rocket, Cpu, ShieldAlert, Database, Hexagon, Network, Zap, Radar } from 'lucide-react'
    ...
    <div className="space-y-6 w-full h-full flex flex-col justify-center items-center">
      <div className="relative w-full h-56 flex items-center justify-center rounded-2xl bg-black border border-white/10 overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.8)]">
         ...
         <div className="relative z-20 w-48 h-48 md:w-56 md:h-56 flex items-center justify-center">
           ...
           <motion.div className="absolute inset-4 rounded-full border-2 border-b-transparent border-[#d2af5a]" animate={{ rotate: ignited ? -360 : 0 }} transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }} />
    ...
    <div className="space-y-6 w-full h-full flex flex-col justify-center">
       <div className="relative flex items-end justify-around h-56 bg-black rounded-2xl border border-white/10 p-4 overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.8)]">
         ...
         <div className="relative w-16 h-full flex items-end justify-center z-30">
           <motion.div className="w-full rounded-t-lg shadow-[0_0_15px_#d2af5a]" style={{ background: 'rgba(201, 148, 58, 0.8)' }} animate={{ height: `${h2}%` }} transition={{ type: 'spring' }} />
           <div className="absolute -bottom-2 bg-black/80 px-2 py-1 rounded backdrop-blur-sm border border-white/10 flex flex-col items-center">
       ...
       <div className="space-y-4 pt-4">
         ...
         <input type="range" min="0" max="100" value={h2} onChange={e => handleH2(Number(e.target.value))} className="w-full h-1.5 rounded bg-[#d2af5a]/30" style={{ accentColor: theme?.primary }} />
       </div>
      ...
      <div className="flex-1 w-full flex items-center justify-center relative mt-6" style={{ transformStyle: 'preserve-3d' }}>
        ...
        <div className={`absolute inset-0 flex items-center justify-center transition-all duration-1000 ${isNested ? 'opacity-0 scale-50' : 'opacity-100 scale-100'}`}>
          <div className="relative w-40 h-40">
            ...
            <div className="absolute bottom-4 left-2 w-24 h-24 rounded-full border-2 border-[#d2af5a] bg-[#d2af5a]/10 flex items-end justify-start pb-4 pl-3 text-[10px] font-bold text-[#d2af5a] mix-blend-screen shadow-[0_0_15px_rgba(201, 148, 58,0.3)]">PEOPLE</div>
        ...
        <div className={`absolute inset-0 flex items-center justify-center transition-all duration-1000 ${!isNested ? 'opacity-0 scale-150 pointer-events-none' : 'opacity-100 scale-100'}`}>
          <div className="relative flex items-center justify-center" style={{ transform: `rotateX(60deg) rotateZ(${rotate}deg)` }}>
            ...
            {/* Sociedade */}
            <div className="absolute w-32 h-32 rounded-full border-2 border-[#d2af5a] bg-[#d2af5a]/10 flex items-center justify-center shadow-[0_0_20px_rgba(201, 148, 58,0.4)]">
              <span className="absolute -left-6 text-[8px] text-[#d2af5a] font-mono tracking-widest" style={{ transform: `rotateZ(-${rotate}deg)` }}>SOCIEDADE</span>
            </div>
            {/* Economia */}
...
export function SimESGScanner({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const pillars = {
    ...
    S: { name: 'SOCIAL', color: '#d2af5a', desc: 'Diversidade, Direitos Humanos, Segurança', risk: 'Risco de Reputação / Boicote de Consumidor' },
...
export function SimFrameworkConstellation({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const frameworks = [
    ...
    { id: 'SASB', target: 'Investidores', focus: 'Impacto do mundo no financeiro da empresa', color: '#d2af5a' },
  ...
  return (
    <div className="flex flex-col h-full w-full justify-between">
      <div className="flex justify-between items-center mb-4">
        <button onClick={() => setGateway('XOR')} className={`px-2 py-1 text-[9px] font-bold font-mono rounded border ${gateway === 'XOR' ? 'bg-[#d2af5a]/20 border-[#d2af5a] text-[#d2af5a]' : 'bg-transparent border-white/20 text-white/50'}`}>XOR (Exclusivo)</button>
        ...
        {tokenActive && (
          <>
            ...
            <div className="absolute top-[30%] left-[86px] w-3 h-3 rounded-full -mt-1.5 shadow-[0_0_10px_currentColor] opacity-0 animate-[tokenPathA_2s_linear_1s_forwards]"
                 style={{ backgroundColor: gateway === 'XOR' ? '#d2af5a' : '#00ffd0', color: gateway === 'XOR' ? '#d2af5a' : '#00ffd0' }} />
...
export function SimCertGlobe({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const certs = [
    ...
    { name: 'Fair Trade', desc: 'Preço Mínimo & Ética', x: '50%', y: '50%', color: '#d2af5a' },
...
export function SimPDCACycle({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const stages = [
    ...
    { name: 'CHECK', color: '#d2af5a', desc: 'Controle e Medição de KPIs', angle: 180 },
...
export function SimBusinessCanvas({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const blocks = [
    ...
    { id: 'Relacionamento', group: 'Receita', color: '#d2af5a' },
    { id: 'Segmentos', group: 'Receita', color: '#d2af5a' },
    { id: 'Recursos', group: 'Custo', color: '#3b82f6' },
    { id: 'Canais', group: 'Receita', color: '#d2af5a' },
...
export function SimStrategicTripod({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const frameworks = [
    ...
    { name: 'MATRIZ BCG', color: '#d2af5a', desc: 'Estrela, Vaca Leiteira, Ponto de Interrogação, Vira-Lata' }
  ]
    ...
    <div className="flex flex-col h-full w-full justify-center items-center">
      ...
      <div className="flex-1 w-full relative flex items-center justify-center perspective-[800px]">
        ...
        <div className="w-40 h-40 relative transition-transform duration-1000" style={{ transformStyle: 'preserve-3d', transform: `rotateY(${face * -120}deg)` }}>
          ...
          {/* Face 2: BCG */}
          <div className="absolute w-40 h-40 border-2 bg-black/80 flex flex-wrap items-center justify-center p-2 backface-hidden"
               style={{ borderColor: '#d2af5a', transform: 'rotateY(240deg) translateZ(35px)', boxShadow: face === 2 ? '0 0 30px rgba(201, 148, 58,0.3)' : 'none' }}>
            <div className="w-1/2 h-1/2 border border-[#d2af5a]/30 flex items-center justify-center text-xl">⭐</div>
            <div className="w-1/2 h-1/2 border border-[#d2af5a]/30 flex items-center justify-center text-xl">❓</div>
            <div className="w-1/2 h-1/2 border border-[#d2af5a]/30 flex items-center justify-center text-xl">🐄</div>
            <div className="w-1/2 h-1/2 border border-[#d2af5a]/30 flex items-center justify-center text-xl">🐕</div>
          </div>
      ...
      <div className="flex-1 w-full flex flex-col items-center justify-center mt-6 gap-2">
        ...
        <div className="w-full max-w-[240px] h-16 border border-white/30 rounded flex items-center justify-between p-1 relative">
           ...
           {['Inbound', 'Ops', 'Outbound', 'MKT', 'Serviço'].map((a, i) => (
             <div key={i} className="flex-1 h-full mx-0.5 bg-[#d2af5a]/20 border border-[#d2af5a]/40 flex items-center justify-center text-[7px] font-bold text-[#d2af5a] rotate-180" style={{ writingMode: 'vertical-rl' }}>
               {a}
             </div>
           ))}
           {/* Flecha de Margem */}
    ...
    <div className="flex flex-col h-full w-full items-center justify-center p-2">
      ...
      <div className="flex w-full gap-2 justify-center">
        ...
        <div className="flex-1 p-2 border border-white/20 rounded bg-white/5 flex flex-col items-center">
          ...
          <div className="w-12 h-12 rounded-full border-2 border-white/20 relative flex items-center justify-center">
            <div className="absolute w-[2px] h-5 bg-[#d2af5a] origin-bottom -mt-5 transition-transform duration-200"
                 style={{ transform: `rotate(${(revenue / 300) * 180 - 90}deg)` }} />
          </div>
          <span className="text-[10px] font-mono mt-2 text-[#d2af5a]">
      ...
      <div className="flex gap-4 items-end justify-center w-full h-32 mt-6">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="w-10 h-24 border border-white/20 rounded-t-lg relative overflow-hidden bg-black/50">
             <div className="absolute bottom-0 w-full bg-[#d2af5a] transition-all duration-500" style={{ height: `${fin}%` }} />
          </div>
          <button onClick={() => { setFin(f => Math.min(100, f + 20)); if(addLog) addLog('Empréstimo captado: Injeção no Fluxo de Financiamento.') }} className="mt-2 text-[8px] px-2 py-0.5 border border-[#d2af5a] text-[#d2af5a] rounded">+ CAPTAR</button>
        </div>
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-black overflow-hidden font-mono text-white/80 p-6">
      ...
      <div className="w-full max-w-md h-48 relative border-b border-l border-white/20 mt-8">
        <svg className="absolute inset-0 w-full h-full" viewBox={`0 0 ${Math.max(years, 1)} ${maxVal}`} preserveAspectRatio="none">
          ...
          <polyline 
            ...
            fill="none" stroke="#d2af5a" strokeWidth="0.1"
            style={{ filter: 'drop-shadow(0 0 4px #d2af5a)' }}
          />
        </svg>
      ...
      <div className="mt-12 flex flex-col items-center gap-2 w-full max-w-md">
        ...
        <input 
          ...
          className="w-full accent-[#d2af5a]"
        />
        <div className="flex justify-between w-full text-[9px] mt-2 border border-white/10 p-2 rounded bg-white/5">
          ...
          <div>EXPONENCIAL (Composto): <span className="text-[#d2af5a] font-bold">R$ {data[years].composto.toFixed(0)}</span></div>
        </div>
      </div>
    </div>
  )
...
export function SimCompoundGravity({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center bg-black overflow-hidden perspective-[800px]">
      ...
      {/* Central Mass */}
      <div className="absolute w-12 h-12 bg-[#d2af5a] rounded-full shadow-[0_0_50px_#d2af5a] flex items-center justify-center z-10">
      ...
      <div className="absolute top-4 left-4 flex flex-col gap-2 z-20">
         ...
         <div className="flex items-center gap-2">
            ...
            <input type="range" min="1" max="30" value={rate} onChange={e => setRate(Number(e.target.value))} className="w-24 accent-[#d2af5a]"/>
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-black overflow-hidden font-mono p-4">
         ...
         <div className="absolute left-0 top-0 bottom-0 w-1/3 flex flex-col justify-around py-4">
            {fc.map((val, i) => (
              <div key={i} className="flex items-center gap-2 px-2">
                 ...
                 <div className="h-[1px] bg-[#d2af5a] flex-1 relative" style={{ opacity: val/500 }}>
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-[#d2af5a] rounded-full blur-sm animate-pulse" />
                 </div>
              </div>
            ))}
         </div>
      ...
      <div className="w-full max-w-lg mt-6 flex justify-between gap-4">
         <div className="flex-1 flex flex-col gap-1">
            ...
            <input type="range" min="1" max="30" value={wacc} onChange={e=>setWacc(Number(e.target.value))} className="accent-[#d2af5a]" />
         </div>
...
export function SimCorporateGyroscope({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const color = isDanger ? '#ff0055' : isHealthy ? '#00ffd0' : '#d2af5a'
...
export function SimMarketEras({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  const eras = [
    { id: 'producao', name: 'ERA DA PRODUÇÃO', desc: 'Foco no produto. Oferta cria sua demanda.', color: '#d2af5a' },
      ...
      <div className="relative w-64 h-64 flex items-center justify-center perspective-[1000px]">
        ...
        {activeEra === 0 && (
          <div className="relative w-full h-full flex items-center justify-center">
             ...
             <div className="absolute w-24 h-4 bg-[#d2af5a]/20 border border-[#d2af5a] rounded flex items-center justify-around overflow-hidden shadow-[0_0_15px_#d2af5a]">
                {Array.from({length: 3}).map((_, i) => (
                  <div key={i} className="w-4 h-4 bg-[#d2af5a] animate-[slide-right_2s_linear_infinite]" style={{ animationDelay: `${i * 0.6}s` }} />
                ))}
             </div>
             <div className="absolute bottom-10 text-[10px] text-[#d2af5a] tracking-widest">PRODUTO PADRONIZADO (FORD T)</div>
          </div>
        )}
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-black overflow-hidden font-mono p-4">
      ...
      <div className="relative w-72 h-72 flex items-center justify-center perspective-[1000px] mt-4" style={{ transform: 'rotateX(40deg)' }}>
        ...
        <div onClick={() => setActiveLayer(1)} className={`absolute w-40 h-40 rounded-full border-2 transition-all cursor-pointer ${activeLayer >= 1 ? 'border-[#d2af5a] shadow-[inset_0_0_20px_rgba(201, 148, 58,0.2)]' : 'border-white/20 hover:border-white/40'}`} />
      ...
      <div className="mt-8 flex gap-4 w-full max-w-lg z-20">
        ...
        <div className={`flex-1 p-3 rounded border transition-all ${activeLayer >= 1 ? 'border-[#d2af5a]/50 bg-[#d2af5a]/10' : 'border-white/10 opacity-30'}`}>
          <div className="text-[10px] text-[#d2af5a] font-bold mb-1">2. DESEJO</div>
...
export function SimLeadershipTheories({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const theories = [
    { name: 'Traços (1940)', desc: 'Carisma inato. (Falhou pois ignora contexto)', color: '#d2af5a' },
      ...
      <div className="w-full max-w-3xl flex items-center justify-between px-8">
        ...
        <div className="relative w-64 h-64 border border-white/20 rounded-full flex items-center justify-center overflow-hidden">
          ...
          {stage === 2 && Array.from({length: 8}).map((_, i) => (
             <div key={i} className="absolute w-4 h-4 bg-[#d2af5a] rounded-sm transition-all duration-1000"
        ...
        <div className="w-1/2 flex flex-col gap-4">
          ...
          <div className="p-4 bg-white/5 border border-white/10 rounded min-h-[100px]">
            ...
            {stage === 2 && <div className="text-[10px] text-[#d2af5a]"><span className="font-bold mb-2 block">Normatização:</span> Regras estabelecidas. Confiabilidade mútua aumenta. A equipe começa a agir como unidade em vez de indivíduos isolados.</div>}
...
export function SimTeamDysfunctions({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const layers = [
    { name: 'Confiança (Vulnerabilidade)', color: '#d2af5a', w: 'w-64' },
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-black font-mono overflow-hidden p-4">
      <div className="absolute top-4 right-4 text-[9px] text-[#d2af5a]">AS 5 DISFUNÇÕES DE EQUIPE (LENCIONI)</div>
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-black font-mono overflow-hidden">
      ...
      <div className="relative w-64 h-64 border border-white/20 flex items-center justify-center mb-8 rounded"
           style={{ 
             borderColor: lens===0 ? '#ff0055' : lens===1 ? '#00ffd0' : lens===2 ? '#d2af5a' : '#a855f7',
        ...
        {/* Virtude: Caráter */}
        {lens === 2 && (
          <div className="flex flex-col items-center text-[#d2af5a]">
            <div className="text-[12px] font-bold mb-2">CARÁTER DA EMPRESA</div>
            <div className="w-24 h-24 border-4 border-[#d2af5a] rounded-full flex items-center justify-center animate-[spin_10s_linear_infinite]">
      ...
      <div className="flex gap-2 z-20">
        ...
        <button onClick={()=>setLens(2)} className={`px-2 py-1 text-[9px] border transition-all ${lens===2 ? 'bg-[#d2af5a]/20 border-[#d2af5a] text-[#d2af5a]' : 'border-white/20'}`}>Virtude</button>
      ...
      <div className="relative w-72 h-48 border border-white/10 mb-8 overflow-hidden bg-white/5">
        ...
        {system === 2 && (
          <div className="absolute inset-0 flex items-center justify-center bg-black">
             ...
             <div className="absolute bottom-0 w-full h-8 bg-gradient-to-r from-[#ff0055] via-[#d2af5a] to-[#00ffd0] animate-pulse" />
          </div>
        )}
      </div>
...
export function SimEasternAesthetics({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-[#0a0a0a] font-mono overflow-hidden">
      <div className="absolute top-4 right-4 text-[9px] text-[#d2af5a]">ESTÉTICA WABI-SABI E WU WEI</div>
      ...
      <div className="relative w-64 h-64 flex items-center justify-center mb-8">
         ...
         >
           ...
           <div className="absolute transition-all duration-700 text-[#d2af5a] text-[12px] font-bold tracking-widest"
      ...
      <div className="w-full max-w-sm flex flex-col items-center">
         <div className="flex justify-between w-full text-[9px] text-[#d2af5a] mb-2">
            <span>Rígido (Controle Ocidental)</span>
            <span>Orgânico (Fluxo Oriental)</span>
         </div>
         <input type="range" min="0" max="100" value={zen} onChange={(e)=>setZen(Number(e.target.value))} className="w-full accent-[#d2af5a]" />
      </div>
...
export function SimCohortStorytelling({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-black font-mono overflow-hidden">
      <div className="absolute top-4 left-4 text-[9px] text-[#d2af5a]">ANÁLISE DE COHORT: RETENÇÃO</div>
      ...
      <div className="flex gap-4">
        ...
        <div className="w-64 border border-white/10 bg-white/5 p-4 text-[10px]">
           <div className="text-[#d2af5a] font-bold mb-2">DATA STORYTELLING</div>
...
export function SimToulminModel({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  const nodes = [
    ...
    { id: 'qualifier', label: 'QUALIFICADOR (Qualifier)', desc: 'Grau de certeza ("Na maioria dos casos...").', pos: 'top-1/4 right-10', color: '#d2af5a' },
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-[#0a0a0a] font-mono overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-6 w-full max-w-sm px-4">
         ...
         <div className="flex flex-col items-center p-2 border border-white/10 bg-black">
            ...
            <div className={`text-[20px] font-black ${desemprego > 10 ? 'text-[#ff0055]' : 'text-[#d2af5a]'}`}>{desemprego.toFixed(1)}%</div>
            <div className="w-full h-1 bg-white/10 mt-2"><div className="h-full bg-[#d2af5a]" style={{ width: `${(desemprego/15)*100}%` }} /></div>
         </div>
...
export function SimBrazilStructuralChallenges({ theme, addLog }: { theme: any, addLog?: (msg: string) => void }) {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-[#0a0a0a] font-mono overflow-hidden">
      <div className="absolute top-4 left-4 text-[9px] text-[#d2af5a]">BRASIL: PARADOXOS ESTRUTURAIS</div>
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center bg-black font-mono overflow-hidden">
      ...
      <div className="flex gap-8 mb-8">
         ...
         <div className="w-64 border border-white/10 bg-white/5 p-4 flex flex-col gap-4">
            ...
            <div>
              <div className="flex justify-between text-[10px] mb-1">
                 <span className="text-[#d2af5a]">Velocidade de Setup</span>
                 <span className="text-white/50">{speed}d</span>
              </div>
              <div className="w-full h-1 bg-white/10"><div className="h-full bg-[#d2af5a] transition-all duration-500" style={{width: `${speed}%`}} /></div>
            </div>
