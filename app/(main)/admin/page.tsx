...
export default function AdminPage() {
  ...
  const blockUser = async (id: string, block: boolean) => {
    ...
    if (profile?.phone && targetUser) {
      const msg = block
        ? `Segurança Business Syllabus: Você bloqueou com sucesso o usuário ${targetUser.name || 'Sem nome'} (${targetUser.email}).`
        : `Segurança Business Syllabus: Você desbloqueou com sucesso o usuário ${targetUser.name || 'Sem nome'} (${targetUser.email}).`
    ...
    <div className="relative min-h-screen text-white px-2 pb-32 pt-16 md:px-4">
      ...
      {tab === 'users' && !loading && (
        <div className="space-y-2">
          ...
          {creatingUser && (
            <div className="rounded-[0.8rem] bs-soft p-3.5 space-y-2.5">
          ...
          <div className="space-y-1 max-h-[60vh] overflow-y-auto scrollbar-hide">
            {filteredUsers.map((u) => (
              <div key={u.id} className="rounded-[0.8rem] bs-soft px-3 py-2.5">
                {editingUser === u.id ? (
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {[{ l: 'Total', v: nonAdminSubs.length, c: '#60a5fa' }, { l: 'Ativos', v: nonAdminSubs.filter((s: SubRow) => s.status === 'active').length, c: '#4ade80' }, { l: 'Trial', v: nonAdminSubs.filter((s: SubRow) => s.status === 'trial').length, c: '#facc15' }, { l: 'Cancel.', v: nonAdminSubs.filter((s: SubRow) => s.status === 'cancelled').length, c: '#fb923c' }, { l: 'Devendo', v: nonAdminSubs.filter((s: SubRow) => s.status === 'overdue').length, c: '#f87171' }].map((s) => (
              <div key={s.l} className="rounded-[0.7rem] bs-soft px-1.5 py-2 text-center"><p className="text-[10px] font-bold" style={{ color: s.c }}>{s.v}</p><p className="text-[8px] text-white/65">{s.l}</p></div>
            ))}
          </div>
          ...
          {subs.filter((s: SubRow) => { const u = users.find(u => u.id === s.user_id); return !u || u.role !== 'admin' }).map((s: SubRow) => { const up = users.find(u => u.id === s.user_id); return (
            <div key={s.id} className="flex items-center gap-3 rounded-[0.8rem] bs-soft px-3 py-2.5">
      ...
      {tab === 'communication' && !loading && (
        <div className="space-y-2">
          {/* Como funciona */}
          <div className="rounded-[0.8rem] bs-soft p-3.5">
      ...
      {tab === 'equipes' && !loading && (
        <div className="space-y-3">
          ...
          {teams.map(team => (
            <div key={team.id} className="rounded-[0.8rem] bs-soft p-3.5 space-y-2.5">
          ...
          {/* Transferir admin */}
          <div className="rounded-[0.8rem] bs-soft border-[#f8717120] bg-[#f8717104] p-3.5 space-y-2.5">
      ...
      {tab === 'config' && !loading && (
        <div className="space-y-2">
          ...
          {Object.entries(configs).map(([key, value]) => (
            <div key={key} className="rounded-[0.8rem] bs-soft px-3 py-2">
            ...
            {activityData && (
              <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
                ...
                {activityData.lastSignIn && (
                  <div className="rounded-[0.7rem] bs-soft px-3 py-2">
