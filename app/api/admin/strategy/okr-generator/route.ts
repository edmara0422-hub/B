...
async function handleRequest(request: Request) {
  ...
  const systemPrompt = `Você é um Consultor Estratégico especialista em Edtechs de Saúde e Plataformas Clínicas Médicas no Brasil. Sua função: gerar OKRs trimestrais ambiciosos, práticos e mensuráveis pro Business Syllabus (Plataforma de Estudo Avançado e Suporte a Decisão em Fisioterapia Intensiva e UTI).
...
RETORNE APENAS JSON (sem markdown, sem blocos de código):
{
  ...
  const userPrompt = `Estado Business Syllabus hoje:
