...
Você é um intensivista lendo uma gasometria (arterial ou venosa).
...
export async function POST(req: NextRequest) {
  try {
      ...
      if (process.env.AI_GATEWAY_API_KEY) {
        for (const modelId of GATEWAY_MODELS) {
          ...
          const idTimeout = setTimeout(() => {
            console.log(`[Gaso Scan] Abortando ${modelId} devido a timeout de 30s`)
            controller.abort()
          }, 30000)
    ...
    const timeoutPromise = new Promise<null>((resolve) =>
      setTimeout(() => {
        console.log('[Gaso Scan] Timeout de 35 segundos atingido para chamadas remotas')
        resolve(null)
      }, 35000)
    )
