...
// Replace { title, content, quote, studyCase, deepDive } with { title, subsections: [{ title, content, quote, studyCase, deepDive }] }
// But we need to do this carefully. Since I know exactly what I appended, I can just replace the whole N1-S* block.
const startIdx = contentM1M8.indexOf('{id: \'N1-S1\''); // Actually it was `  {\n    id: 'N1-S1',`
const startRegex = /\{\s*id:\s*'N1-S1',/;
const match = startRegex.exec(contentM1M8);

if (match) {
  const start = match.index;
  const end = contentM1M8.indexOf('{', contentM1M8.indexOf('id: \'M4-S1\'', start)); // or simply indexOf the M4-S1 block.
  // Wait, M4-S1 block is after N1-S4.
  const m4s1Idx = contentM1M8.indexOf('id: \'M4-S1\'');
  const m4s1Start = contentM1M8.lastIndexOf('{', m4s1Idx);
...
`;
  ...
  const newContent = contentM1M8.slice(0, start) + neuroContentStr.trim() + ",\n  " + contentM1M8.slice(m4s1Start);
  fs.writeFileSync('data/caderno-content-m1-m8.ts', newContent);
  console.log('Fixed Neuro DB format');
} else {
  console.error('Could not find N1-S1 block');
}
